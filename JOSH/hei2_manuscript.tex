...
% shorthand declarations in this document
...
%For instance \cite{Tay08} applied GP on a terminal set of job-features for 
%flexible job-shop with promising results compared to the benchmarks DRs from 
%the 
%literature.
%In fact, \cite{Hildebrandt2010} revisited the experiments from \cite{Tay08} 
%that were reported to be successful for dynamic job-shop and tested it against 
...
%representative for real-world long-term job-shop applications, e.g., by the 
...
%correctify some possibly bad dispatch made previously (sort of reverse 
%lookahead). Their method computationally inexpensive, although the authors do 
%stress that there is still remains room for improvement.
...
The problem space consists of job-shop s in semiconductor manufacturing, with 
...
The training data is necessarily also problem instance specific. %and must 
%reflect 
...
\cref{sec:DR} describes the main featues for job-shop, 
...
A commonly used subclass of FSP in the literature is permutation flow-shop, 
which 
...
The class of problem instances used in our studies is the job-shop scheduling 
...
\end{eqnarray}
\end{subequations}
Note, that this is given that there is only made one non-optimal dispatch. 
...
case scenario of making one suboptimal dispatch (i.e. 
...
the stepwise optimality evolves for its intended trajectory, thereby updating 
\cref{eq:tracc:opt} to:
...
trying out different combinations of DRs, however, it is non-trivial. In this 
section one approach to learning how such combinations is presented.
...
In this study, then it's known that $r \propto C_{\max}^{\pi_\star}$, hence the 
optimal job-list is the following:
\begin{equation}\quad
...
% pi_i^hat: is prfef model from prev. iteration
\end{equation}
...
\cref{tbl:IL:stats}. As one can see, DAgger is not fruitful when the same 
...
with each iterations. Which from \cref{sec:pil:expr} showed it was not a 
...
A summary of \jrnd{10}{10} best PIL and AIL models w.r.t. \namerho, from 
...
The \sdr s remain a popular approach to scheduling, as they are simple to 
implement and quite efficient. 
Nevertheless, when they are successful and when they fail remains illusive. 
%investigation of how these \dr s actually work, and what 
...
\cref{fig:case} clearly illustrated. 
...
This also opens up the question of how should validation accuracy be measured? 
...
This would simplify the training set, making it (presumably) have less 
...
need to be especially taken care off. 
\Cref{fig:case} showed how making suboptimal decisions is especially critical 
...
trajectories as well, since the future observations depend on previous 
predictions. 
...
%feature-states that need to be optimised for their correct labelling. Making 
%it only practical for job-shop of a considerably small size, or only few 
%iterations.
...
%expert policy is only queried when needed in order to reduce computational
%cost. 
%This occurs when a metric of a newly encountered schedules are sufficiently 
%large enough from previously queried schedules (to ensure diversity of 
%learned optimal schedules). 
%Moreover, in DAgger all data samples are equally important, irrespective of 
%its iteration, which can require great number of iterations to learn how to 
%recover from the mistakes of earlier policies. To address the naivety of the 
%data aggregation, MMD suggests only aggregating a new data point if it is 
%sufficiently different to previously gathered schedules, and if the current 
%policy has made a mistake. 
%Additionally, there are multiple policies, each specialising in a particular 
%region of the feature-space where previous policies made mistakes.
%Although MMD has better empirical performance (based on robot applications), 
%it requires defining metrics, which in the case of job-shop is non-trivial 
%(cf. 
...
%aggressively queries the expert, making it impractical for some problems, 
