...
\todo[inline]{Reorganize as follows:\\
4.1: \st{Problem Instances: the instances, the performance criteria,}\\
4.2: Reconstructing optimal solutions, placement rules, probability of choosing 
optimal for different SDR \\
4.3: \st{Blended DR} \\
4.4: \st{Best worst case analysis for wrong decision}\\
4.5: Summary, $\rho$ versus classification accuracy. Very critical to choose 
data set correctly...
}
...
\;|\;\forall (J_o,J_s) \in \mathcal{O}^{(k)} \times \mathcal{S}^{(k)}
