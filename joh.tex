...
this study gives a framework on how to analyse heuristics performance for 
scheduling problems.  
By researching optimal schedules there is an abundance of valuable 
...
Furthermore, generally stepwise optimality (or classification accuracy) implies 
good end performance, here minimising the final makespan. 
...
Using an expert policy, a preference set of schedules is collected and a 
preference based learning model is created based on what feature states are 
preferable to others.
Moreover, the learned model should be updated in an active  imitation learning 
fashion, such that learned state spaces properly represent ones that the model 
would eventually be encountered.
With this in mind it's possible to learn new \cdr s that outperform the models 
their features were based on. 
...
Machine Learning \and Preference Learning \and Imitation Learning \and DAgger}
\end{abstract}

\section{Introduction}\label{sec:introduction}

Hand crafting heuristics for scheduling is a ad-hoc approach to finding approximate solutions to NP hard problems. The process is time-consuming and its performance can vary dramatically between different problem instances. The aim of this work is to investigate how this process can be automated. In particular the task will be to learn new problem specific priority \dr s for subclass of scheduling problems known as the \jsp\ scheduling problem (JSP). 
\JSP\ deals with the allocation of tasks of competing resources where its goal is to optimise a single or multiple objectives.  Its analogy is from manufacturing industry 
where a set of jobs are broken down into tasks that must be processed on 
several machines in a workshop. Furthermore, its formulation can be applied on a wide variety of practical problems in real-life applications which involve decision making, therefore its
problem-solving capabilities has a high impact on many manufacturing organisations.
...
from heuristic design to supervised learning to data creating (data distributed as it is used). Understanding the critical points in the construction of solution ...
...
\xi^\star_{\pi} := \mathbb{E}\left\{\pi_{\star} = \pi \;\mid\; \pi_{\star} 
\right\}
\end{equation}
...
From \cref{fig:opt:SDR:xistar} one observed that $\xi^\star_{\text{MWR}}$ has a 
higher probability than random guessing, in choosing a dispatch which may 
result in an optimal schedule. This is especially true towards the end of the 
schedule building process. 
Similarly, the $\xi^\star_{\text{LWR}}$ chooses dispatches 
...
\begin{figure}
  \centering
  \includegraphics[width=1\linewidth]{figures/{trdat_prob_moveIsOptimal_10x10_SDR_xistar}.pdf}
...
\xi_{\pi} := \mathbb{E}\left\{\pi_{\star} = \pi \;\mid\; \pi \right\}
\end{equation}
...
is not statically significant from MWR (boost in mean $\rho$ 
is at most 0.5\%). 
...
\begin{figure}
  \centering
  \includegraphics[width=1\linewidth]{figures/j_rnd/{trdat_prob_moveIsOptimal_10x10_SDR_xi}.pdf}
...
Note, as the following \lcnamecref{sec:il:passive}s require repeated collection 
of training data, and since its labelling is a very time intensive task the 
remainder of the paper will solely be focusing on \jrnd{10}{10}.
...
MWR after $T=2$ iterations (by $\approx5\%$).
