...
\todo[inline]{The introduction must be completely re-written to reflect what was done in this paper.)}
...
Better understanding of how and when dispatching rules work. How to setup the learning problem.
...
$\mathcal{M}=\{M_a\}_{a=1}^m$, of $m$ machines. The index $j$ refers to a job $J_j\in\mathcal{J}$ while the index  $a$ refers to a machine $M_a\in\mathcal{M}$. If a job requires a number of 
...
The jobs are subject to the  constraint that each job $J_j$ must follow a predefined machine order, a chain or sequence of $m$ operations 
...
these will not be considered here. Once an operation is started, it must be completed uninterrupted, i.e., pre-emption is not allowed. Moreover, there are no sequence dependent 
...
\todo[inline]{Here we must say something about the computational complexity of the $J||C_{\max}$ compared to other scheduling problems, and why we consider only this one in our study. It may be useful to look at Pinedo also on this subject ... perhaps also mention that many of the dispatching rules in the literature have been designed around this problem... use this to tie to he next section}


\section{Priority Dispatching Rules} \label{sec:constructionjssp}


Priority dispatching rules determine, from a list of incomplete jobs, which job should be dispatched next. This process is illustrated in \Cref{fig:jssp:example}, where an example of 
a temporal partial schedule of six-jobs scheduled on five-machines is given. The numbers 
...
$C_{\max}$ is denoted by a dotted vertical line. The object is to keep this value as small as possible once all operations are complete. As shown in the example there are $15$ operations already scheduled. The \textit{sequence} of dispatches used to create this partial schedule is,
\begin{eqnarray}
...
the sequence is determined from the available jobs to be scheduled. In our example
...
job as soon as it becomes available and such that the machine idle time is minimal. 
...
is independent of the priority dispatching rule applied.\todo{But clearly they are not...}
...
\begin{figure}[t!]\centering
	\includegraphics[width=0.8\textwidth]{figures/jssp_example_nocolor.pdf}
...
which is reported in order to distinguish which features may be use to determine whether any two jobs are competing for a place on the same machine;\todo{How is this features used in your code?}
...
\todo[inline,color=orange!40]{$@$Helga: not sure how this works again... check 
    ...
    \\$@$Helga: lets delete the last two features from the table, they are anyway not makespan related and not used. The mathematical symbols in the table have not been described? Are they then needed?}
...
rules are static during the entire scheduling process, however, ties could also be broken randomly. \todo{We break randomly?}
...
job-shop. Two different processing times distributions were explored, namely \jrnd{n}{m}  
