...
\todo[color=pink]{Note, this is not the \textit{common}  \textbf{permutation} 
flow-shop, but rather FIXED job-shop}
...
\todo[inline,color=red]{Here we must say something about the computational 
complexity of 
...
section \\
Put text here that talks about NP hard and why we need heuristics to solve this 
problem, one such heuristic is the dispatching rules which are introduced in 
the following section. What us the maximum size we can solve 14x14 specialized 
branch and bound ...}
\todo[inline,color=green]{I've already discussed the NP-hardness in the 
introduction. Should I repeat that or rather move it down here?}
...
address the possible negative impact an operation\todo{Is this an $(j,a)$ operation?} might have on other machines 
...
\vfill
\todo[inline]{Remove page break later!}
\pagebreak % for now ... remove later 
...
where $\vec{p}\sim\mathcal{U}(1,99)$. These jobs are denoted by \frnd{n}{m} and is analogous to 
\jrnd{n}{m}.
...
\subsection{Reconstructing Optimal Solutions}\label{sec:reconstruct}
...
For JSP there are $d=\NrFeatLocal$ features (cf. \cref{tbl:jssp:feat}), and the 
training set is created in the manner described in \cref{sec:learnCDR}.
...
dispatches to choose from, and the number of possible dispatches decrease over 
time.
%\Cref{fig:opt:unique} depicts how the number of 
...
%As the number of possible dispatches decrease over time, \cref{fig:opt} 
%depicts the probability of choosing an optimal dispatch. 
