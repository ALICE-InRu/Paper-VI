...
%Other prevalent approach to solving \JSP\ is to combine several relatively 
%\sdr s such that they may benefit each other for a given problem space. 
%% automation for selecting DRs #3 - GA
%To automate that selection, \cite{InRu14}, translated \dr s into measurable 
%attributes (or features) and optimised what their contribution should be via 
%evolutionary search. %The framework is straight forward and easy to implement 
%and showed promising and robust results, as models were trained on a lower 
%dimension, and validated on higher dimension. 
