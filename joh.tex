...
\begin{abstract}
  ...
  \begin{enumerate*}
    ...
    data distribution $\mathcal{D}$. This is done by updating the learned model in 
...
In this particular instance, one cannot infer that choosing $J_1$ is better 
...
the highest preference estimate, i.e., \cref{eq:jstar} where $\pi(\cdot)$ is the 
...
performance \cite{CesaBianchi06,Hannan57}, which is the inspiration for our next 
...
with a $\Delta\rho\approx-4.24\%$ boost in mean performance. Otherwise, for \ref{bias:equal} instead, there is a loss of $\Delta\rho\approx+6.23\%$ in mean performance. 
...
enough variance between $\Phi^{\text{IL}i}$ and $\Phi^{\text{IL}(i-1)}$, hence the aggregated feature set 
...
is needed, as  $\Delta\rho\approx-11.68$ for $i=1$, and after that it stagnates with $\Delta\rho\approx+0.55\%$ for $i=2$ and for $i=3$ it significantly worse
than the previous iteration by $\Delta\rho\approx+0.75\%$
\end{enumerate*}
...
a considerably small size, or only few iterations.
...
As mentioned before, the main drawback of DAgger is that it quite aggressively 
