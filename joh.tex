...
%\renewcommand{\vphi}{\boldsymbol\phi}
...
\begin{abstract}
	...
	their explanatory predictability. From which, a preference set is collected 
	and 
	a preference based learning model is created based on what feature states are 
	preferable to others w.r.t. the end result, here minimising the final 
	makespan.
	...
	Even though this study is based around the job-shop scheduling problem, it 
	can 
...
\todo[inline]{The introduction must be completely re-written to reflect what 
...
Hardness, connect to what has bee done before ... test-bed for learning 
heuristics ...

\todo{Helga working on the literature review here}
Group the literature on learning heuristics. I have moved this material from 
old chapter 4 to the intro I think it will flow better, we then say we will 
concentrate on the last approach here:
\begin{itemize}
...
heuristics, dispatching rules, although the techniques developed could be 
adapted to improvement
heuristics also. 


\todo{What are   your contributions?}  
Better understanding of how and when dispatching rules work. How to setup the 
learning problem.
...
$M_a\in\mathcal{M}$. If a job requires a number of 
processing steps or operations, then the pair $(j,a)$ refers to the operation, 
i.e., processing the task of job $J_j$ on machine $M_a$. 
The jobs are subject to the  constraint that each job $J_j$ must follow a 
predefined machine order, a chain or sequence of $m$ operations 
...
these will not be considered here. Once an operation is started, it must be 
completed uninterrupted, i.e., pre-emption is not allowed. Moreover, there are 
no sequence dependent 
set-up times. The objective will be to schedule the jobs 
so as to minimize the maximum completion times for all tasks, also known as the 
makespan, $C_{\max}$.  This family of scheduling problems is denoted by
$J||C_{\max}$ \citep{Pinedo08}.  

%In the case when all jobs share the same 
...
%Therefore, without the loss of generality, this study will be structured 
%around 
%the JSP.

\todo[inline]{Here we must say something about the computational complexity of 
...
subject ... perhaps also mention that many of the dispatching rules in the 
...
Priority dispatching rules determine, from a list of incomplete jobs, which job 
should be dispatched next. This process is illustrated in 
\Cref{fig:jssp:example}, where an example of 
...
$C_{\max}$ is denoted by a dotted vertical line. The object is to keep this 
value as small as possible once all operations are complete. As shown in the 
example there are $15$ operations already scheduled. The \textit{sequence} of 
dispatches used to create this partial schedule is,
\begin{eqnarray}
	\vchi=\left(J_3,J_3,J_3,J_3,J_4,J_4,J_5,J_1,J_1,J_2,J_4,J_6,J_4,J_5,J_3\right)
\end{eqnarray}
and refers to the sequential ordering of job dispatches to machines, i.e., 
$(j,a)$; 
...
the sequence is determined from the available jobs to be scheduled. In our 
example
the available jobs is given by the job-list
...
is independent of the priority dispatching rule applied.\todo{But clearly they 
are not...}
Different placement rules could be considered, for example placing a job 
in the smallest feasible slot. In our experiments cases were discovered where
 such a placement can rule out the possibility of constructing optimal
solutions.
This problem, however, did not occur when jobs are simply placed as early as 
possible. For this reason it will be the placement rule applied here.
...
Priority dispatching rules will use attributes of operations, such as 
processing time, 
...
\begin{table}[t!] \centering
	\caption[Attribute space $\mathcal{A}$ for JSP]{Attribute space $\mathcal{A}$ 
		for JSP where job $J_j$ on machine $M_a$ given the resulting temporal 
		schedule after operation $(j,a)$.
	}
	\label{tbl:jssp:feat}
...
which is reported in order to distinguish which features may be use to 
determine whether any two jobs are competing for a place on the same 
machine;\todo{How is this features used in your code?}
...
\todo[inline,color=orange!40]{$@$Helga: not sure how this works again... check 
    ...
    \\$@$Helga: lets delete the last two features from the table, they are 
    anyway not makespan related and not used. The mathematical symbols in the 
    table have not been described? Are they then needed?}
...
be broken randomly. \todo{We break randomly?}
...
\todo{Take one iteration over this paragraph with Helga}
...
Instances}\label{sec:gentrainingdata}

In this section we will describe issues that must be addresses when learning 
new priority dispatching rules. At the same time we will describe the 
experimental setup used in our experimental studies. Basically:

\begin{itemize}
\item Problem instance generator; the problem instances considered and their 
optimal solution
\item Re-constucting the optimal solutions using a priority dispatching rules. 
\item Generating training data.
\item Attribute selection and machine learning technique.
\item Testing and validation.
\end{itemize}

The experimental set-up is given in the following 
\lcnamecref{sec:data:sim}, starting with description of problem spaces in 
...
optimal makespan. 
Generation of training data is described in \Cref{sec:gentrdata}, in particular 
...
\todo{Not done, but could be. More valid for $6\times5$ data, which we're 
currently not using.}
\Cref{sec:liblinear} illustrates how the linear learning is set up, and 
...
where $I=[1,99]$ and \jrndn{n}{m} where $I=[45,55]$. Note, the problem spaces 
%\jrnd{n}{m} and \jrndn{n}{m} 
