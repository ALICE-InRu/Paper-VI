...
between consecutive jobs $J_j$ and $J_{j'}$ is called idle time, or flow, 
...
feature in order to measure the quality of the schedule. 
...
%\textrm{ if } x_f(j,\vsigma_j(a-1)\geq x_f(j',a) 
\end{eqnarray}
...
i.e.,
\begin{eqnarray}
\tilde{S}_{ja}&:=&\{J_{j'}\in \mathcal{J}_a\;|\;\tilde{s}(a,j')\geq p_{ja} 
\}\label{eq:slots}.
\end{eqnarray} 
...
inspected: First slot chosen could always achieve its known optimum, however 
only $97\%$ of the instances when choosing the smallest slot.} and henceforth 
