...
For \jrnd{10}{10}  there is no statistical difference between models (1.3, 1.6) 
w.r.t. $\rho$ and validation accuracy. 
Same goes for (2.69, 3.355, 3.358, 3.524), except only the latter three w.r.t. 
...
w.r.t. validation accuracy, however, they have considerable performance 
difference w.r.t. $\rho$ ($\Delta\rho \approx 18\%$). 
...
Other models were statistically significant to one another, using a 
...
For \frnd{10}{10} there are more or less no (or minimal) statistical difference 
w.r.t. $\rho$, only exception is (1.6, 1.7).
Although, w.r.t. validation accuracy models there are a few equivalent models, 
...
\subsection{Interpreting CDRs}\label{sec:CDR:interpret}
...
As was stated before, there is no statistical difference between \jrnd{10}{10} 
models (2.69, 3.355, 3.358, 3.524). As \cref{fig:CDR:weights} 
shows, \phiendTime, \phijobWrm\ and \phimacWrm\ are similar in value, and in 
the case of 3.358, then \phimacFree\ has similar contribution as the 
\phiendTime\ for the other models. 
Which, as standalone models are 1.6 and 1.3, respectively, and yield equivalent 
mean $\rho$
...
counterpart.\footnote{Note, 
  \phiendTime$~\leq~$\phimacFree, where \phiendTime$~=~$\phimacFree\ implies 
  $J_j$ is the latest job on $M_a$, otherwise placed in a previously created 
  slot on $M_a$.} 
...
%However, looking at model 3.502 which has a slight statistical difference 
%w.r.t. accuracy, the third feature yields the staggering difference in 
%performance, about 20\% increase in $\rho$. 

It's interesting to inspect the full model for \frnd{10}{10}, 16.1. 
Despite having similar contributions as all the active features as (3.213, 
3.216), then the (slight) interference from of other features, hinders the full 
model from achieving a low $\rho$. 
...
\phimacFree, so there are some slots created, which could be better utilised.
