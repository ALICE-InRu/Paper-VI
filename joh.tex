...
\cref{fig:opt:SDR}. These probabilities
vary quite a bit between the different problem instances distributions studied. 
From \cref{fig:opt:SDR} one observed that $\xi_{\text{MWR}}$ has a higher 
...
\end{equation}
\Cref{fig:opt:SDR:jrnd} shows the difference between $\xi_{\langle \text{SDR} 
\rangle}$ and $\hat{\xi}_{\langle \text{SDR} \rangle}$ for \jrnd{10}{10}.
