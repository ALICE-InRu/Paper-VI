...
implement, fast and find reasonable schedules. In addition, they are relatively easy 
to interpret, which makes them desirable for the end-user.
...
to their machine order. Unfinished jobs are dispatched one at a time according to 
...
\cref{fig:boxplot:BDR}. \todo{just use \jrnd{10}{10}}
...
\todo{add random dispatching on this figure, does it perform like random?}
...
achieve on its own. 
 \todo{this last paragraph ends with the hope the CDRs will perform better than BDRs, and so motivate the next section, we also observe here that there will no be one SDR that fits all problem distributions and that custom build ones will be required for each instance distribution.}
...
$\vpsi^{o}=\vphi^{o}-\vphi^{s}$, and suboptimal, 
$\vpsi^{s}=\vphi^{s}-\vphi^{o}$ by $y_o=+1$ and $y_s=-1$ respectively.  
...
the highest preference estimate, i.e.,
\begin{equation}\label{eq:lin}
...
limited to $\abs{\Psi}\leq 500,000$ by random sampling.
