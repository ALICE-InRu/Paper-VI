...
Put text here that talks about NP hard and why we need heuristics to solve this problem, one such heuristic is the dispatching rules which are introduced in the following section. What us the maximum size we can solve 14x14 specialized branch and bound ...
...
$J_{j'}$ is called idle time, or flow, \todo{Show me where in the literature where the term flow is used for this?}
\begin{equation} 
...
space in a logical manner, respecting not to exclude the optimum\todo{Are you sure about this, I don't think this is right!}. Here, the 
construction heuristic is to schedule the dispatches as closely together as 
possible, i.e., minimise the schedule's flow\todo{...}. 
...
\section{Learning New Priority Dispatching Rules}\label{sec:learnCDR}
