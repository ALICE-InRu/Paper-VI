...
\title{Discovering dispatching rules from data and imitation learning}
...
%is to optimise a single or multiple objectives.  Its analogy is from 
%manufacturing industry 
%where a set of jobs are broken down into tasks that must be processed on 
%several machines in a workshop. Furthermore, its formulation can be applied on 
%a wide variety of practical problems in real-life applications which involve 
%decision making, therefore its
...
However, this method cannot outperform the original \dr\ for the data 
generation. 
...
not sufficient. Once these rules make a suboptimal dispatch the are in 
uncharted 
...
%their contribution should be via evolutionary search. The framework is 
%straight 
%forward and easy to implement and showed promising and robust results, 
%as models were trained on a lower dimension, and validated on higher 
%dimension. 
...
%    Flexible \jsp\ allows jobs to be processed on different machines, i.e., 
%the 
...
different processing times, machine ordering, and dimensions. Each instance 
will 
...
average for a given distribution of problem instances. To illustrate the 
performance 
...
These box-plots show the difference in performance of the various SDRs. The MWR 
performs on average the best on the \jrnd{n}{m} and \jrndn{n}{m} problems 
instances, whereas for \frnd{n}{m} is is LWR that performs best. It is also 
interesting to observe that all but the MWR perform statistically worse than 
...
\begin{eqnarray} 
\zeta_{\min}^{\star}(k) &:=& \mathbb{E}_{\pi_\star}\left\{\min(\rho)\;:\;
\forall C_{\max}^{\vchi^j} \gneq C_{\max}^{\pi_\star} 
\wedge J_j\in\mathcal{L}^{(k)} \right\} \\
\zeta_{\max}^{\star}(k) &:=& \mathbb{E}_{\pi_\star}\left\{\max(\rho) \;:\;
\forall C_{\max}^{\vchi^j} \gneq C_{\max}^{\pi_\star}
\wedge J_j\in\mathcal{L}^{(k)} \right\}
\end{eqnarray}
\end{subequations}
Note, that this is given that one makes \emph{one} non-optimal dispatch. 
...
used or not, i.e., 
$\abs{\Phi^{\text{DA}2}}\approx\abs{\Phi^{\text{DA}i}_{\text{EXT}}}$.
...
\end{enumerate*}
As a reference, the \sdr\ MWR is shown on at the edges of 
...
For instance, the before training the learned model, the preference set $\Psi$ 
needs to be re-sampled to size $l_{\max}$. 
As the effects of making suboptimal choices varies as a function of times 
steps, the stepwise bias should rather be done proportional to the mean 
