...
only applicable slots are those idle times that can process the entire 
operation, namely:
\begin{equation}\quad
...
in terms of the operation belonging to the same time step $k$, with the 
exception of \phijobTotProcTime\ and \phimacTotProcTime\ which are static for a 
given problem instance but vary for each $J_j$ and $M_a$, respectively. 
...
job list, in such a manner that there is more than one 
...
Nevertheless, when they are successful and when they fail remains elusive. 
