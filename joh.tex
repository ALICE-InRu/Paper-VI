...
\begin{abstract}
  Recent efforts to creating \dr s have focused on direct search methods and learning from scheduling data.
  This paper will examine the latter approach and present a     systematic approach for doing so effectively. 
  The key to learning an effective dispatching rule is through the careful construction of the training data,  
  ...
  However, as the impact of suboptimal moves is not fully understood, the labelling must be adjusted for its intended trajectory.
 
  Using the guidelines set by the framework the design of custom \dr s, for  one's particular scheduling application, will be more effective. In the study  presented three different distributions of the \jsp\ will be considered. The machine learning approach considered is based on preference learning, 
...
\dr. 
However, the learned method cannot outperform the original \dr\ used for the data 
generation. 
...
The training data is necessarily also problem instance specific. %and must reflect 
...
Moreover, a supervised learning algorithm will optimize classification accuracy, while it is the actual end-performance of the dispatching rule learned that will determine the success of the learning method. 
...
machines at a later time, it is for that reason attributes such as 
...
As mistakes are bound to made at some points, it is interesting to see how the 
stepwise optimality evolves for its intended trajectory, thereby updating 
...
However, after $k>20$ then the BDR performance starts diverging from that of MWR. 
...
trying out different combinations of DRs, however, it is non-trivial. In this section one approach to learning how such combinations is presented.
...
%sufficiently different to previously gathered states, and if the current 
...
%it requires defining metrics, which in the case of \jsp\ is non-trivial (cf. 
